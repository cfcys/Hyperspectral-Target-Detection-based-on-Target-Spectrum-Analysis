...
class ECEM(Detector):  # ECEM的大类

    def __init__(self):
        Detector.__init__(self)
        self.windowsize = [1/4, 2/4, 3/4, 4/4]  # window size
        self.num_layer = 10  # the number of detection layers 检测层数的数字
        self.num_cem = 6  # the number of CEMs per layer    每一层中cem的数字
        self.Lambda = 1e-6  # the regularization coefficient  正则化系数(用于防止过拟合)
        self.show_proc = True  # show the process or not  (是否去展示这个过程)
